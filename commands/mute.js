...
exports.run = (client, message, args) => {
  ...
  // mute role check
  if (!muterole) {
    guild.roles.create({name: "mute"});
    guild.channels.forEach(c => c.overwritePermissions(guild.roles.find(r=>r.name === "mute"), {
      SEND_MESSAGES: false,
      ADD_REACTIONS: false,
  ...
  const embed = new Discord.MessageEmbed()
    .setAuthor(member.user.tag, member.user.avatarURL({ format: "png", size: 1024 }))
    .setTitle("User Muted")
    .setFooter("User Muted", bot.avatarURL({ format: "png", size: 1024 }))
    .setTimestamp()
